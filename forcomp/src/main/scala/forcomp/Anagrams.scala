...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c => c).toList.map(c => (c._1, c._2.length)).sortBy(c => c._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = List() :: {
    for {
      ...
      growingHead <- occurrences take split
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x.foldRight(List[(Char, Int)]())((z, agg) => {
      val newCount = z._2 - yMap.getOrElse(z._1, 0)
      if (newCount > 0)
        (z._1, newCount) :: agg
      else
        agg
